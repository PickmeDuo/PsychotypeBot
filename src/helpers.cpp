...
namespace Psycho {
    ...
    std::string PsychologicalAnalyzerBot::analyzeGroup(const int64_t chat_id) {
        if (tendency.empty()) {
            return "Нет данных для анализа";
        }

        std::string result;
        for (auto& name : groupsData[chat_id]) {
    ...
    std::string PsychologicalAnalyzerBot::resultDescription(const std::string& psychotype) {
        if (psychotype == "Пикми") {
            ...
            return "Тревожно-мнительный — это человек с повышенной чувствительностью к возможным угрозам и склонностью постоянно анализировать свои действия, переживать о последствиях. Осторожный, предусмотрительный, стремится избежать ошибок или неодобрения окружающих. Такой человек сомневается в себе, много думает перед принятием любого решения, нередко пересматривает в голове прошедшие события, чтобы убедиться, что всё сделал правильно. Он соблюдает правила, избегает риска, предпочитает заранее продумывать запасные планы. При этом человек может быть добросовестным, внимательным к деталям и ответственным, но из-за внутреннего напряжения часто испытывает усталость и тревогу. Он болезненно воспринимает критику, боится осуждения, нуждается в поддержке и ощущении безопасности. Несмотря на внешнюю застенчивость, внутри него - буря переживаний, из которой рождается стремление быть хорошим, не подвести, не причинить никому вреда.";
    ...
    void PsychologicalAnalyzerBot::onGroupMessage(const std::string& message, const std::string& username, const int64_t chat_id) {
        bool new_user = true;
        for (auto s : groupsData[chat_id]) {
            if (s == username) {
                new_user = false;
                break;
            }
